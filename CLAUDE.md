...
1. **`src/data/content.ts` is the single source of truth.** It exports `contents` (`Record<Lang, Content>`, `en`/`sv`) plus a `ui` dict of static UI strings; `content` is the canonical English copy that tests and defaults use. The Swedish object *derives from* `en` (spread + text overrides) so slugs/URLs/stacks can't drift. Components pick a language via `useLang()` (`src/lib/useLang.ts`, localStorage-persisted, toggled in the Nav); the terminal reads `contents[getLang()]` at command time but its own chrome stays English. When adding fields, update the interfaces at the top of the same file and both language objects.
